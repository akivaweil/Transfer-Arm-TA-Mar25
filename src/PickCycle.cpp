...
2. Lower Z-axis for Pickup (Continuous, Activate Vacuum during descent):
  ...
  - Vacuum (SOLENOID_RELAY_PIN HIGH) activates when Z passes
Z_SUCTION_START_POS.
...
6. Move to Dropoff Overshoot Position: X-axis to X_DROPOFF_OVERSHOOT_POS (4
inches past dropoff).
...
15. Return to Pickup Position (X-axis to X_PICKUP_POS, Servo to
SERVO_PICKUP_POS).
16. Home X-axis.
17. Final Move to Pickup Position (X-axis to X_PICKUP_POS). Cycle ends, enters
WAITING state.
*/
//* ************************************************************************
//* ************************* PICK CYCLE LOGIC ***************************
//* ************************************************************************
// This file implements the state machine for the pick-and-place cycle of the
// Transfer Arm. It controls the sequence of movements, servo operations, and
// vacuum activation/deactivation.

// State variables
PickCycleState currentState = WAITING;
unsigned long stateTimer = 0;
bool midpointServoRotated =
    false;  // Track if servo has been rotated at midpoint
bool vacuumActivatedDuringDescent =
    false;  // Track if vacuum activated during Z descent for pickup
...
void updatePickCycle() {
  ...
  switch (currentState) {
    ...
    case LOWER_Z_FOR_PICKUP:
      ...
      // This state is entered once zStepper.moveTo(Z_PICKUP_POS) has been
      // called.
    ...
    case LOWER_Z_FOR_DROPOFF:
      ...
      transferArm.getZStepper().setMaxSpeed(
          Z_DROPOFF_MAX_SPEED);  // Set slower speed for dropoff movement
