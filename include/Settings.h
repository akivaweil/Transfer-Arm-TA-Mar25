...
const int STEPS_PER_REV =
    400;  // Steps per revolution for steppers (1.8° with 1/2 microstepping)
...
const float STEPS_PER_MM =
    STEPS_PER_REV /
    (PULLEY_TEETH * BELT_PITCH);  // Steps per mm of linear movement
...
const int Z_DROPOFF_LOWER_INCHES =
    5.5;  // Lower Z-axis by 5.5 inches for dropoff
...
const int X_DROPOFF_OVERSHOOT_POS =
    X_DROPOFF_OVERSHOOT_INCHES * STEPS_PER_INCH;  // Overshoot position in steps
const int X_SERVO_ROTATE_POS =
    X_SERVO_ROTATE_INCHES *
    STEPS_PER_INCH;  // Position to start servo rotation for dropoff
const int X_MIDPOINT_POS =
    X_MIDPOINT_INCHES * STEPS_PER_INCH;  // Kept for reference

const int Z_UP_POS = 0;  // Z-axis fully up position
const int Z_PICKUP_POS =
    Z_PICKUP_LOWER_INCHES * STEPS_PER_INCH;  // Z-axis down position for pickup
const int Z_SUCTION_START_POS =
...
const int SERVO_TRAVEL_POS =
    0;  // Servo position for travel after pickup (in degrees)
const int SERVO_DROPOFF_POS =
    80;  // Servo dropoff position (90 degrees from pickup)

// Timing constants
const unsigned long PICKUP_HOLD_TIME =
    300;  // Hold time at pickup position (300ms)
const unsigned long DROPOFF_HOLD_TIME =
    100;  // Hold time at dropoff position (100ms)
const unsigned long SERVO_ROTATION_WAIT_TIME =
    500;  // Wait time for servo to complete rotation at overshoot position
          // (500ms)
...
const int Z_DROPOFF_ACCELERATION =
    Z_ACCELERATION / 1;         // Same acceleration for now for dropoff
